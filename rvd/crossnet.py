...
class GLSTM(nn.Module):
    def __init__(self, hidden_size=256, groups=4):
        super(GLSTM, self).__init__()
   
        hidden_size_t = hidden_size // groups
     
        self.lstm_list1 = nn.ModuleList([nn.LSTM(hidden_size_t, hidden_size_t//2, 1, batch_first=True, dropout=0.25, bidirectional=True) for i in range(groups)])
        self.lstm_list2 = nn.ModuleList([nn.LSTM(hidden_size_t, hidden_size_t//2, 1, batch_first=True, dropout=0.25, bidirectional=True) for i in range(groups)])
...
class RandomChunkPositionalEncoding(nn.Module):
    def __init__(self, F, C, Tmax = 4500): # T_max=2000
        """
        :param channels: The last dimension of the tensor you want to apply pos emb to.
        """
...
class CrossNet(nn.Module):
    ...
    def __init__(
        ...
        # decoder
        self.decoder = nn.Linear(in_features=num_freqs*dim_hidden, out_features=dim_output)
    ...
    def forward(self, x: Tensor, return_attn_score: bool = False) -> Tensor:
        ...
        x = x.permute(0, 2, 1, 3)
        x = x.reshape(B, T, F * x.shape[3])
        y = self.decoder(x).permute(0, 2, 1)
        if return_attn_score:
